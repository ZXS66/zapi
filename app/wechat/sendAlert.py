...
from app.constants import WECHAT_APPID, WECHAT_APPSECRET, WECHAT_TEMPLATEID, WECHAT_TOUSERS
...
def __getAccessToken():
    resp = requests.get(f"https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={WECHAT_APPID}&secret={WECHAT_APPSECRET}").json()
    return AccessTokenWithExpiration(resp["access_token"], resp["expires_in"], datetime.now())
...
async def __sendAlert(access_token:str, touser:str, template_id:str, af:AlertForm):
    ...
    return requests.post(f"https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={access_token}", json=data)
...
async def send(af:AlertForm):
    ...
    if (__access_token.obtained_at + timedelta(seconds=__access_token.expires_in - 8)) < datetime.now():
